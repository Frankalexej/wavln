...
if __name__ == "__main__":
    ...
    if test_name.split("-")[0] in ["reconloss"]: 
        resresdict = {}
        for test_type in ["trained", "C"]: 
            if test_type == "trained": 
                res_save_dir = os.path.join(model_save_, f"eval-{train_name}-{ts}")
            else: 
                res_save_dir = os.path.join(model_save_, f"eval-cross{test_type}-{train_name}-{ts}")
        ...
        res_save_dir = os.path.join(model_save_, f"eval-{train_name}-{ts}")
        plot_many([resresdict["trained"], resresdict["C"]], ["trained-sashi", "C-lari"], 
