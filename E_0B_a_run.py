...
if __name__ == "__main__": 
    ...
    if args.dataprepare: 
        ...
        mk(guide_path)
        generate_separation(os.path.join(src_, "phi-fathi-F-guide.csv"), 
                            os.path.join(src_, "phi-fathi-Th-guide.csv"), 
                            guide_path, 
                            nameset={"larger": "F", "smaller": "Th"})
        
        with open(os.path.join(model_save_dir, "README.note"), "w") as f: 
            f.write("----------------RUN NOTES----------------\n")
            f.write("E_0B (20241120): We use FATHI dataset; and we shall use higher input dimension and higher time resolution. \n")
