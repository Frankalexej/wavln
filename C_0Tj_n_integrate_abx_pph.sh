...
zls=('ASPALLSTOP')    # 'POS' 'VC' 'STOPPOA' 'PARC-0-2' 'PARC-2-4' 'PARC-4-6' 'PPP' 'PPH' 'GENDER'
...
ts='1021123825'
tn="FinalEpochs" # FinalEpochsDimneutral
...
for m in "${ms[@]}"; do
    for c in "${cs[@]}"; do
        for zl in "${zls[@]}"; do
            ...
            python C_0Tj_n_integrate_abx_pph.py -ts "$ts" -m "$m" -cd "$c" -gpu "$gpu" -zl "$zl" -tn "$tn"&
        done
    done
done
