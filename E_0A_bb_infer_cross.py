...
def run_one_epoch(model, single_loader, both_loader, model_save_dir, stop_epoch, res_save_dir, hiddim, 
    ...
    for (x, x_lens, pt, s_name, v1_name, v2_name, sf1, sf2, phoneseq, sid, gender) in both_loader: 
        # name = name[0]
    ...
    plot_spectrogram(all_ori[i].T, title=f"mel-spectrogram of input {all_pre_names[i]}-{all_stop_names[i]}-{all_vowel_names[i]}", ax=axs[0])
    plot_spectrogram(all_recon[i].T, title=f"reconstructed mel-spectrogram {all_pre_names[i]}-{all_stop_names[i]}-{all_vowel_names[i]}", ax=axs[1])
    fig.tight_layout()
...
if __name__ == "__main__":
    ...
    if not PU.path_exist(os.path.join(model_save_, f"{dataset_train_name}-{ts}")):
        raise Exception(f"Training {dataset_train_name}-{ts} does not exist! ")
