...
# we have very limited data, so we don't need to select, just plot all
def get_toplot_ptk(hiddens, sepframes, phi_types, stop_names, offsets=(0, 1)): 
    cutstarts = []
    cutends = []
    for hidden, sepframe, phi_type in zip(hiddens, sepframes, phi_types):
        if phi_type == 'ST':
    ...
    hid_sel = np.empty((0, 8))
    tag_sel = []
    for (item, start, end, tag) in zip(hiddens, cutstarts, cutends, stop_names): 
        hid = cutHid(item, start, end, offsets[0], offsets[1])
        hidlen = hid.shape[0]
        hid_sel = np.concatenate((hid_sel, hid), axis=0)
        tag_sel += [tag] * hidlen
...
if __name__ == "__main__":
    ...
    for run_number in range(1, 11):
        ...
        for epoch in range(0, 100):     
            ...
            hidr_cs, tags_cs = get_toplot_ptk(hiddens=all_zq, 
                                            sepframes=all_sepframes,
                                            phi_types=all_phi_type,
                                            stop_names=all_stop_names,
                                            offsets=(0.4, 0.6))
    ...
    plot_silhouette(asp_sil_lists, stop_sil_lists, os.path.join(res_save_dir, f"silhouette-VS-{args.condition}.png"))
    print("Done.")