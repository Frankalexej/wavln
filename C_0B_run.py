# This is the integrated runner of training and evaluation. 
# This includes three conditions: unbalanced, balanced and no T training. 
# For each run, we will randomly select speakers and generate training and validation guide files. 
# The three conditions will be trained with the same separation. 
######################### Libs #########################
import torch
...
from model_dataset import TargetVowelDataset as ThisDataset
...
def load_data(rec_dir, target_path, load="train", select="both", sampled=True):
    if sampled: 
    ...
    ds = ThisDataset(rec_dir, 
                        integrated,  
                        transform=mytrans)

    use_shuffle = True if load == "train" else False
    loader = DataLoader(ds, batch_size=BATCH_SIZE, shuffle=use_shuffle, num_workers=LOADER_WORKER, collate_fn=ThisDataset.collate_fn)
    return loader
...
def run_once(hyper_dir, model_type="ae", condition="b"): 
    ...
    if condition == "u":
        train_loader = load_data(rec_dir, guide_path, load="train", select="both", sampled=False)
        valid_loader = load_data(rec_dir, guide_path, load="valid", select="both", sampled=False)
    elif condition == "b":
    ...
    for epoch in range(num_epochs):
        ...
        for idx, (x, x_lens, pt, sn) in enumerate(onlyST_valid_loader):
            current_batch_size = x.shape[0]
            x_mask = generate_mask_from_lengths_mat(x_lens, device=device)
