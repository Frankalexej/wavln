...
ts='1113024340'     # cosine loss trained, default init, AEPPV9, lr=5e-4, noise=0.004
echo "Timestamp: $ts"
...
#             python E_0A_bb_infer_cross_lari.py -ts "$ts" -rn "$i" -m "$m" -cd "$c" -gpu "$gpu" &
#         done
#     done
# done
...
for task in "${tasks[@]}"; do 
    ...
    gpu=$((RANDOM % 9))
    python E_0A_bb_infer_cross_lari.py -ts "$ts" -rn "$i" -m "recon$mnum-phi" -cd "$c" -gpu "$gpu" &
