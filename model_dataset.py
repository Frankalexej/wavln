...
class TargetVowelDatasetBoundaryPhoneseqSpeakerGenderManualNorm(Dataset): 
    # this mixes T and ST. 
    # Target means the phenomenon-target, that is, e.g. /th/ or /st/. 
    # NOTE: for TV condition we add silence as # in the place of S
    # NOTE: this version does not generate random silence here, because for evaluation 
    ...
    def __getitem__(self, idx):
        ...
        return (mel_data, self.phi_type[idx], self.stop_name[idx], self.vowel_name[idx], 
    ...
    def collate_fn(data):
        # only working for one data at the moment
        batch_first = True
        xx, pt, sn, vn, sf1, sf2, phoneseq, sid, gender = zip(*data)
        x_lens = [len(x) for x in xx]
        xx_pad = pad_sequence(xx, batch_first=batch_first, padding_value=0)
        return xx_pad, x_lens, pt, sn, vn, sf1, sf2, phoneseq, sid, gender

class TargetVowelDatasetBoundaryPhoneseqTESTManualNorm(Dataset): 
    # this mixes T and ST. 
    # Target means the phenomenon-target, that is, e.g. /th/ or /st/. 
    # NOTE: for TV condition we add silence as # in the place of S
    # NOTE: this version does not generate random silence here, because for evaluation 
    ...
    def __getitem__(self, idx):
        mel_data, this_phi_type, this_stop_name, this_phone_seq, wav_data = self.___loaditem(idx)

        if self.normalizer: 
            mel_data = self.normalizer(mel_data, self.mean, self.std)
        
        return (mel_data, self.phi_type[idx], self.stop_name[idx], 
                self.vowel_name[idx], self.first_sep_frame[idx], self.second_sep_frame[idx], 
                this_phone_seq, wav_data)
    ...
    def ___loaditem(self, idx):
        ...
        return data_transformed, self.phi_type[idx], self.stop_name[idx], phoneseq, data

    @staticmethod
    def collate_fn(data):
        # only working for one data at the moment
        batch_first = True
        xx, pt, sn, vn, sf1, sf2, phoneseq, wav = zip(*data)
        x_lens = [len(x) for x in xx]
        xx_pad = pad_sequence(xx, batch_first=batch_first, padding_value=0)
        return xx_pad, x_lens, pt, sn, vn, sf1, sf2, phoneseq, wav


class TargetVowelDatasetBoundaryPhoneseqSpeakerGender(Dataset): 
    # this mixes T and ST. 
    # Target means the phenomenon-target, that is, e.g. /th/ or /st/. 
    # NOTE: for TV condition we add silence as # in the place of S
    # NOTE: this version does not generate random silence here, because for evaluation 
    # NOTE: silence can be generated outside. 
    def __init__(self, src_dir, guide_, select=[], mapper=None, transform=None, plosive_suffix="", hop_length=400, 
                 noise_amplitude_scale=0.01, speaker_meta_path=None):
        # guide_file = pd.read_csv(guide_)
        if isinstance(guide_, str):
            guide_file = pd.read_csv(guide_)
        elif isinstance(guide_, pd.DataFrame):
    ...
    def __getitem__(self, idx):
        ...
        return (data, self.phi_type[idx], self.stop_name[idx], self.vowel_name[idx], 
                self.first_sep_frame[idx], self.second_sep_frame[idx], 
                phoneseq, self.speaker[idx], self.gender[idx])
...
class TargetVowelDatasetBoundaryPhoneseqTEST(Dataset): 
    # this mixes T and ST. 
    # Target means the phenomenon-target, that is, e.g. /th/ or /st/. 
    # NOTE: for TV condition we add silence as # in the place of S
    # NOTE: this version does not generate random silence here, because for evaluation 
    ...
    def __getitem__(self, idx):
        ...
        return transformed_data, self.phi_type[idx], self.stop_name[idx], self.vowel_name[idx], self.first_sep_frame[idx], self.second_sep_frame[idx], phoneseq, data
...
class TargetVowelDatasetBoundaryPhoneseqSpeaker(Dataset): 
    # this mixes T and ST. 
    # Target means the phenomenon-target, that is, e.g. /th/ or /st/. 
    # NOTE: for TV condition we add silence as # in the place of S
    # NOTE: this version does not generate random silence here, because for evaluation 
    ...
    def __init__(self, src_dir, guide_, select=[], mapper=None, transform=None, plosive_suffix=""):
        ...
        guide_file["first_sep_frame"] = guide_file.apply(lambda x: time_to_frame(x['stop_startTime'] - x['pre_startTime'], sr=16000, hop_length=256), axis=1)
        guide_file["second_sep_frame"] = guide_file.apply(lambda x: time_to_frame(x['vowel_startTime'] - x['pre_startTime'], sr=16000, hop_length=256), axis=1)
        ...
        self.first_sep_frame = first_sep_frame_col.tolist()
        self.second_sep_frame = second_sep_frame_col.tolist()
        self.speaker = speaker_col.tolist()
        self.src_dir = src_dir
        self.transform = transform
    ...
    def __getitem__(self, idx):
        ...
        return data, self.phi_type[idx], self.stop_name[idx], self.vowel_name[idx], self.first_sep_frame[idx], self.second_sep_frame[idx], phoneseq, self.speaker[idx]

    @staticmethod
    def collate_fn(data):
        # only working for one data at the moment
        batch_first = True
        xx, pt, sn, vn, sf1, sf2, phoneseq, speaker = zip(*data)
        x_lens = [len(x) for x in xx]
        xx_pad = pad_sequence(xx, batch_first=batch_first, padding_value=0)
        return xx_pad, x_lens, pt, sn, vn, sf1, sf2, phoneseq, speaker
...
class MelSpecTransformDBAugust(nn.Module): 
    def __init__(self, sample_rate, n_fft=400, n_mels=64, normalizer=None, denormalizer=None): 
        ...
        self.inverse_mel = torchaudio.transforms.InverseMelScale(sample_rate=sample_rate, n_mels=n_mels, n_stft=d2p1, f_max=sample_rate//2)
        self.grifflim = torchaudio.transforms.GriffinLim(n_fft=n_fft, hop_length=hop_length)
        self.amplitude_to_DB = torchaudio.transforms.AmplitudeToDB(stype='power')
...
class NormalizerMVNManual(nn.Module):
    def __init__(self): 
        super().__init__()
