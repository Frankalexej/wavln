"""
E is training on full-set data and testing on part, but with additional phoneme prediction task. 
This time we plan to do this: model input is mel and encoder is the same. There are two tasks: 
reconstruction and phoneme prediction. For the first, just follow old code. For the second even older 
code. And to link them is very simple: just do training together. (Because the model by itself is linked)
"""

"""
Test the effect of alpha in loss. 
"""
######################### Libs #########################
import torch
...
def run_once(hyper_dir, model_type="ae", condition="b"): 
    ...
    if model_type == "mtl":
        ...
        masked_loss = MaskedLoss(loss_fn=nn.MSELoss(reduction="none"))
        ctc_loss = nn.CTCLoss(blank=mymap.encode("BLANK"))
        model_loss = PseudoAlphaCombineLoss_Pred(masked_loss, ctc_loss, alpha=alpha)
