"""
E_0A: We use SASHI dataset; and we shall use higher input dimension and higher time resolution. 
- Main modification: 
    -- New data file. 
    -- New configs for Mel-transformation. Similarly, we should use new configs for normalization. 
    -- Similar model. 

E_0D: No difference other than using only one LSTM layer. 
"""
######################### Libs #########################
...
if __name__ == "__main__": 
    ...
    if args.dataprepare: 
        ...
        with open(os.path.join(model_save_dir, "README.note"), "w") as f: 
            f.write("----------------RUN NOTES----------------\n")
            f.write("E_0D (20241210): We use SASHI dataset; and we shall use higher input dimension and higher time resolution. BUT we use only one LSTM layer\n")
