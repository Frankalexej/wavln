...
if __name__ == "__main__": 
    ...
    train_df, val_df, test_df = split_dataset_by_speaker(os.path.join(args.guide_path, args.in_file_name), train_size=0.7, val_size=0.15, test_size=0.15)
    train_df.tocsv(os.path.join(args.guide_path, "training_" + args.in_file_name), index=False)
    val_df.tocsv(os.path.join(args.guide_path, "validation_" + args.in_file_name), index=False)
    test_df.tocsv(os.path.join(args.guide_path, "testing_" + args.in_file_name), index=False)