...
ts='1113024340'
tn="ABXSomething-third-data-SSH" # FinalEpochsDimneutral FinalEpochs ABXSomething-vowel-vowel-speaker
...
for m in "${ms[@]}"; do
    for c in "${cs[@]}"; do
        for zl in "${zls[@]}"; do
            ...
            python E_0A_tests.py -ts "$ts" -m "$m" -cd "$c" -gpu "$gpu" -zl "$zl" -tn "$tn"&
        done
    done
done
