"""
E_2A: We use SASHI dataset; but we will use full phone sequence prediction, thus we shall train the model SUPERVISED. 
- Main modification: 
    Compared to CTC training, we will use the full phone sequence as the target, and the model will encode Mel and the decoder will do decoding autoregressively. 
    In fact, the model can just work on encoded info to predict the full phone sequence timestep by timestep, but to keep the model consistent, 
    we will use last timestep's prediction as the input to the next timestep. But teacher forcing will be used intermittently. 

    - Model: nothing to change. Just change loss function and use full phone sequence as target. 
"""
######################### Libs #########################
...
def run_once(hyper_dir, model_type="ae", condition="b", nameset={"larger": "T", "smaller": "ST"}, noise_controls={"fixlength": False, "amplitude_scale": 0.01}): 
    ...
    for epoch in range(1, num_epochs + 1): 
        ...
        for idx, ((mel_input, pred_target, pred_full_target), (mel_input_lens, pred_target_lens, pred_full_target_lens)) in enumerate(train_loader):
            ...
            # pred_output = pred_output.permute(1, 0, 2)  # As required by the CTCLoss  # NOTE: Not used here because we use MaskedCrossEntropyLoss
            ...
            loss, l_reconstruct, l_embedding, l_commitment = pred_loss.get_loss(pred_output, pred_full_target, mel_mask), torch.tensor(0), torch.tensor(0), torch.tensor(0)
