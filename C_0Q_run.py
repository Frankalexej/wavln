"""
Core changes for H is increasing hidden representation dimension
to 100, according to Chung et al., 2016. 
...
NOTE: Get back to 8 dim. change encoder to unidirectional, 
this is because we want to feed in the enc_hid instead of all zero 
hidden into decoder as starter. 

NOTE: Up to 100. 

K 
NOTE: new model structure

O
NOTE: 5 layers, 8 dims. 

P
NOTE: we give up all about the AE learning and turn to the CTC learning.
We will use modified phi-set to train the model.

Q
NOTE: we will use the same model structure as P, but we will shrink the dictionary size to include 
only related tokens. 
probably we will delete SIL, because this is not a phoneme. And we will see what is the effect of it. 
"""
######################### Libs #########################
import torch
...
N_MELS = 64
LOADER_WORKER = 32
PLOSIVE_SUFFIX = "h"    # separate the plosive phonemes with aspiration

# def load_dict(path=os.path.join(src_, "no-stress-seg.dict")): 
#     with open(path, "rb") as file:
#         mylist = pickle.load(file)
#         mylist = ["BLANK"] + mylist
#         mylist = mylist + ["SIL"]
#         # add the asperated plosives, use h to denote that. This will not overlap with the original phoneme set. 
...
def load_dict(path=None): 
    if path: 
        ...
        guide_st = pd.read_csv(os.path.join(src_, "phi-ST-guide.csv"))
        guide_t = pd.read_csv(os.path.join(src_, "phi-T-guide.csv"))

        mylist = guide_st["stop"].unique().tolist()
        mylist = mylist + guide_t["stop"].unique().tolist()
        mylist = mylist + guide_st["vowel"].unique().tolist()
        mylist = mylist + guide_t["vowel"].unique().tolist()
...
def run_once(hyper_dir, model_type="ae", condition="b"): 
    ...
    if model_type == "mtl":
        ...
        # 20240609: pp-phi is when PP model is trained on phenomenon dataset and tested on phenomenon dataset.
        batch_size = 32
        masked_loss = MaskedLoss(loss_fn=nn.MSELoss(reduction="none"))
        ctc_loss = nn.CTCLoss(blank=mymap.encode("BLANK"))
        model_loss = PseudoAlphaCombineLoss_Pred(masked_loss, ctc_loss, alpha=0.2)
