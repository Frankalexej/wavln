...
TD is running sPV/PV again with new noise. 
"""
######################### Libs #########################
...
if __name__ == "__main__": 
    ...
    if args.dataprepare: 
        ...
        with open(os.path.join(model_save_dir, "README.note"), "w") as f: 
            f.write("----------------RUN NOTES----------------\n")
            f.write("New harmonic noise\n")
            f.write("Others same as T\n")

    else: 
        print(f"{train_name}-{ts}")
        torch.cuda.set_device(args.gpu)
        run_once(model_save_dir, model_type=args.model, condition=args.condition, 
                 nameset={"larger": "T", "smaller": "ST"}, noise_controls={"fixlength": False, "amplitude_scale": 0.006})