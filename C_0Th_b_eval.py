...
from model_model import AEPPV1, AEPPV2, AEPPV4, AEPPV10
...
def run_one_epoch(model, single_loader, both_loader, model_save_dir, stop_epoch, res_save_dir, hiddim): 
    ...
    for (x, x_lens, pt, sn, vn, sf1, sf2, phoneseq, sid, gender) in both_loader: 
        ...
        all_other_hid_outs["enc-rnn1-l"] += [enc_hid_outs[2][:, :, hiddim:].cpu().detach().numpy().squeeze()]
        all_other_hid_outs["enc-rnn2-f"] += [enc_hid_outs[3][:, :, :hiddim].cpu().detach().numpy().squeeze()]
        all_other_hid_outs["enc-rnn2-b"] += [enc_hid_outs[3][:, :, hiddim:].cpu().detach().numpy().squeeze()]
        all_other_hid_outs["enc-rnn2-l"] += [enc_hid_outs[4][:, :, hiddim:].cpu().detach().numpy().squeeze()]
        all_other_hid_outs["enc-rnn3-f"] += [enc_hid_outs[5][:, :, :hiddim].cpu().detach().numpy().squeeze()]
        all_other_hid_outs["enc-rnn3-b"] += [enc_hid_outs[5][:, :, hiddim:].cpu().detach().numpy().squeeze()]
        all_other_hid_outs["enc-rnn3-l"] += [enc_hid_outs[6][:, :, hiddim:].cpu().detach().numpy().squeeze()]
        all_other_hid_outs["enc-rnn4-f"] += [enc_hid_outs[7][:, :, :hiddim].cpu().detach().numpy().squeeze()]
        all_other_hid_outs["enc-rnn4-b"] += [enc_hid_outs[7][:, :, hiddim:].cpu().detach().numpy().squeeze()]
        all_other_hid_outs["enc-rnn4-l"] += [enc_hid_outs[8][:, :, hiddim:].cpu().detach().numpy().squeeze()]
        all_other_hid_outs["enc-rnn5-f"] += [enc_hid_outs[9][:, :, :hiddim].cpu().detach().numpy().squeeze()]
        all_other_hid_outs["enc-rnn5-b"] += [enc_hid_outs[9][:, :, hiddim:].cpu().detach().numpy().squeeze()]
...
def main(train_name, ts, run_number, model_type, model_save_dir, res_save_dir, guide_dir, word_guide_, 
    ...
    if model_type in ["recon4-phi", "recon8-phi", "recon16-phi", "recon32-phi", 
        ...
        model = AEPPV10(enc_size_list=enc_list, 
