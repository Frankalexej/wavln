...
from model_dataset import TargetVowelDatasetBoundaryPhoneseqSpeaker as ThisDataset
from model_dataset import SilenceSampler_for_TV
from C_0X_defs import *
from C_0U_a_run import REC_SAMPLE_RATE, N_FFT, N_MELS, LOADER_WORKER, DROPOUT, INTER_DIM_0, INTER_DIM_1, INTER_DIM_2, INPUT_DIM, OUTPUT_DIM
...
def run_one_epoch(model, single_loader, both_loader, model_save_dir, stop_epoch, res_save_dir): 
    ...
    for (x, x_lens, pt, sn, vn, sf1, sf2, phoneseq, speaker) in both_loader: 
        # name = name[0]
        ...
        (x_hat_recon, x_hat_attn), (attn_w_recon, attn_w_preds), (ze, zq) = model.attn_forward(x, x_lens, x_mask)
        ...
        if np.any(np.isnan(attn_w_recon)): 
            ...
            continue
        if sf1 == sf2: 
            print(f"sf1 = sf2 at {stop_epoch}!")
            continue
        if sf2 == attn_w_recon.shape[0]: 
            print(f"sf2 = end at {stop_epoch}!")
            continue
    ...
    reshandler.res["speaker"] = all_speaker
    reshandler.save()
