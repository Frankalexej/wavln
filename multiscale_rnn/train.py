### This is the second file from the main.
# Three custom packages: `model`, `data`, and `utils`, are imported.
...
def prepare_data(seqs_data):
    ...
    for idx, sentence in enumerate(seqs_data):
        # Populate the data array with the values from the current sentence by slicing the array to the length of the sentence
        # and filling in the rest with 0s
        data[:lengths_data[idx], idx] = sentence
        # Populate the mask array with 1s up to the length of the current sentence and fill in the rest with 0s
        mask[:lengths_data[idx], idx] = 1.
        # Populate the target array with the values from the current sentence starting from the second element (to "roll" to the next word)
        # and adding a 0 at the end to represent the end of the sequence. This is typical of RNNs, but may not be necessary in this case.
...
def train(data_path=["../data/train.tok", "../data/valid.tok"], dict_path="../data/dict.pkl",
    ...
    for epoch in range(max_epoch):
        ...
        # initialize hidden values, this is typical of older RNN models, because by then
        # the hidden values are not automatically initialized
        ...
        for batch in train:
            ...
            if batch is None:
                ...
                it -= 1
                continue
            # seems to be because of type mismatch between output hidden and input
            hidden = repackage_hidden(hidden)
            optimizer.zero_grad()  # 0.0001s used
            inputs, target, mask = prepare_data(batch)  # 0.002s used
            # loss contained in model, PAY ATTENTION
...
def train_PTB(size_list=[512, 512], dict_size=10000, embed_size=650, batch_size=80, maxlen=100,
    ...
    for epoch in range(max_epoch):
        ...
        for it, batch in enumerate(range(0, train_data.size(0) - 1, maxlen)):
            ...
            # NOTE: this model is different, here the 3rd var is hidden, not mask
