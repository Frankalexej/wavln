"""
E_0B: FATHI experiment
20241120: At the moment we do not train models on this dataset. But we will use the trained model from A to test on this dataset.
"""
######################### Libs #########################
...
def generate_separation(larger_path, smaller_path, target_path, nameset={"larger": "T", "smaller": "ST"}): 
    ...
    (training_smaller, valid_smaller), (training_larger, valid_larger), (training_sampled_larger, valid_sampled_larger) = random_sample_by_speaker(larger_guide, smaller_guide, valid_proportion=prepare_configs["valid_proportion"])
...
if __name__ == "__main__": 
    ...
    if args.dataprepare: 
        ...
        mk(guide_path)
        generate_separation(os.path.join(src_, "phi-lari-R-guide.csv"), 
                            os.path.join(src_, "phi-lari-L-guide.csv"), 
                            guide_path, 
                            nameset={"larger": "R", "smaller": "L"})
        
        with open(os.path.join(model_save_dir, "README.note"), "w") as f: 
            f.write("----------------RUN NOTES----------------\n")
            f.write("E_0C (20241120): We use LARI dataset; and we shall use higher input dimension and higher time resolution. \n")
