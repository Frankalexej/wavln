...
def plot_elbows(array, save_path, title="Elbow Method For Optimal k"): 
    ...
    mean_run = np.mean(array, axis=1) # mean over runs of each k
    sem_run = sem(array, axis=1)
...
if __name__ == "__main__": 
    ...
    for (model_type, model_condition, zlevel) in itertools.product(ms, cs, zs): 
        print(f"Processing {model_type} in {model_condition} with {zlevel}...")
        ...
        sse_list_runs = []  # list for each run of lists of sse for each epoch
        ...
        for run_number in learned_runs:
            ...
            for epoch in range(0, 100): 
                ...
                sse_list_epochs.append(sse)
            sse_list_runs.append(sse_list_epochs)

        sse_arr = np.array(sse_list_runs)

        # sse_arr_reshaped = sse_arr.transpose(0, 2, 1).reshape(-1, sse_arr.shape[1])
