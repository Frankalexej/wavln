...
ts='1210012614'     # cosine loss trained, default init, AEPPV9, lr=5e-4, noise=0.004
echo "Timestamp: $ts"
...
#             python E_0D_b_infer.py -ts "$ts" -rn "$i" -m "$m" -cd "$c" -gpu "$gpu" &
#         done
#     done
# done
...
for task in "${tasks[@]}"; do 
    ...
    gpu=$((RANDOM % 9))
    python E_0D_b_infer.py -ts "$ts" -rn "$i" -m "recon$mnum-phi" -cd "$c" -gpu "$gpu" &
