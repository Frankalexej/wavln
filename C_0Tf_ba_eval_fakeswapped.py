...
"""
This is fake swapped test: well-formedness test but just swap within the category. 
"""
...
if __name__ == "__main__":
    ...
    if args.dataprepare: 
        ...
        swapped_st_valid = shuffle_within_condition_and_speaker(
                            df=st_valid,
                            speaker_col='speaker',
        ...
        swapped_t_valid = shuffle_within_condition_and_speaker(
                            df=t_valid,
                            speaker_col='speaker',
                            condition_col='phi_type',
                            plosive_col='stop',
        ...
        swapped_st_valid.to_csv(os.path.join(guide_dir, f"{condition_dict['smaller']}-valid-fakeswapped.csv"), index=False)
        swapped_t_valid.to_csv(os.path.join(guide_dir, f"{condition_dict['larger']}-valid-sampled-fakeswapped.csv"), index=False)
    else:
        model_type = args.model
        model_save_dir = os.path.join(model_save_, f"{train_name}-{ts}-{rn}", args.model, args.condition)
        res_save_dir = os.path.join(model_save_, f"evalfakeswapped-{train_name}-{ts}")
        ...
        main(train_name, ts, rn, model_type, model_save_dir, this_model_condition_dir, guide_dir, valid_full_guide_path, 
            ...
            st_guide_path=os.path.join(guide_dir, f"{condition_dict['smaller']}-valid-fakeswapped.csv"), 
            t_guide_path=os.path.join(guide_dir, f"{condition_dict['larger']}-valid-sampled-fakeswapped.csv"))