...
def plot_attention_epoch_trajectory_diff(all_phi_type, all_attn, all_sepframes1, all_sepframes2, save_path, conditionlist=["ST", "T"]): 
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(24, 8))
    ...
    for (selector, ax, legend_names) in zip(conditionlist, [ax1, ax2], legend_namess):
        ...
        for epoch in range(len(all_attn)): 
            ...
            st_diff = target_group2 - target_group1
            ta_diff = target_group4 - target_group3

            means = np.array([np.mean(st_diff, axis=0), 
                            np.mean(ta_diff, axis=0)])
            
            sems = np.array([sem(st_diff, axis=0),
                            sem(ta_diff, axis=0)])
        ...
        diff_legend_names = ["S|P", "P|V"]
        for mean, upper, lower, label, c in zip(means, upper_bounds, lower_bounds, diff_legend_names, colors):
            ax.plot(mean, label=label, color=c)
            ax.fill_between(range(len(mean)), lower, upper, alpha=0.2, color=c)
        ax.set_xlabel('Epoch')
        ax.set_ylabel('Summed Foreign-Attention')
...
if __name__ == "__main__":
    ...
    plot_attention_epoch_trajectory(every_phi_types, every_attns, every_sepframes1, every_sepframes2, os.path.join(res_save_dir, f"attnepochtraj-diff-at-all-{model_type}-{model_condition}-{strseq_learned_runs}-10pc.png"), 
                                    conditionlist=["S", "Sh"])
