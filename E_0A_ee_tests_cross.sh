...
ts='1113024340'
tn="ABXSomethingAll-second-data-SSH" # FinalEpochsDimneutral FinalEpochs ABXSomething-vowel-vowel-speaker
cmarker="C"
...
for m in "${ms[@]}"; do
    for c in "${cs[@]}"; do
        for zl in "${zls[@]}"; do
            ...
            python E_0A_ee_tests_cross.py -ts "$ts" -m "$m" -cd "$c" -gpu "$gpu" -zl "$zl" -tn "$tn" -cmarker "$cmarker"&
        done
    done
done
