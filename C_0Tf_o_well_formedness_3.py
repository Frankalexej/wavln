...
if __name__ == "__main__":
    ...
    root_ = "../"
    another_root_ = "../../wavlnnew/"
    model_save_ = root_ + "model_save/"
    another_model_save_ = another_root_ + "model_save/"
    ...
    if test_name.split("-")[0] in ["reconloss"]: 
        ...
        resresdict = {}
        for test_type in ["normal", "swapped", "fakeswapped"]: 
            if test_type == "normal": 
                res_save_dir = os.path.join(another_model_save_, f"eval-{train_name}-{ts}")
            elif test_type == "swapped": 
                res_save_dir = os.path.join(another_model_save_, f"evalswapped-{train_name}-{ts}")
            elif test_type == "fakeswapped": 
            ...
            for epoch in range(0, 101): 
                ...
                for run_number in learned_runs:
                    ...
                    sim = cosine_similarity_axis(hidr_ori, hidr_recon, axis=1)
                    asp_list_runs += sim.tolist()
        ...
        res_save_dir = os.path.join(model_save_, f"eval-{train_name}-{ts}")
        plot_many([resresdict["normal"], resresdict["swapped"], resresdict["fakeswapped"]], ["Normal", "Swapped", "Fake Swapped"], 
